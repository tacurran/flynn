...
func (s *SchedulerSuite) SetUpSuite(t *c.C) {
	...
	t.Assert(err, c.IsNil)
	s.slugrunnerID = r.Processes["app"].Env["SLUGRUNNER_IMAGE_ID"]
	t.Assert(s.slugrunnerID, c.Not(c.Equals), "")
...
func (s *SchedulerSuite) createApp(t *c.C) (*ct.App, *ct.Release) {
	...
	artifact := &ct.Artifact{Type: "docker", URI: "https://registry.hub.docker.com/flynn/slugrunner?id=" + s.slugrunnerID}
...
func (s *SchedulerSuite) addHosts(t *c.C, count int) []string {
	...
	for i := 0; i < count; i++ {
		res, err := clusterAPI.PostForm(args.ClusterAPI, url.Values{})
		if err != nil {
